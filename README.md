# Qwen3.8-27B inference endpoint — globus3 (DGX Spark GB10)

An OpenAI-compatible vLLM endpoint for ~6 occasional users doing agentic/coding work.
Launched through slurm, one GPU, model on local NVMe. Clients reach it by SSH-tunnelling
to globus1 — the address does not change when the job moves between nodes.

## Operations

Everything goes through one command (`serving`, already on `$PATH` via `~/.local/bin`):

```bash
serving start          # launch and wait until it is actually answering
serving status         # job state, reachability, live load, MTP acceptance
serving stop           # drain in-flight requests, then stop
serving restart        # stop, wait for memory to be released, start
serving logs -f        # follow the current job's log
serving test           # 8-check smoke test against the live endpoint
serving keys list | add <name> | revoke <name>
serving doctor         # diagnose a broken or unreachable endpoint
```

`serving doctor` is the one to reach for when something is wrong — it distinguishes "no job",
"still loading" (cold start is ~5 min), "publisher tunnel never came up", and "healthy",
and it reads the real exit cause out of the last log rather than making you grep.

A cron entry runs `serving supervise` every 5 minutes. It is silent when healthy, and
restarts only when the job is genuinely gone or has been up long enough to have finished
loading but is failing health checks. Its actions are logged to `logs/supervise.log`.

Hand new users [QUICKSTART.md](QUICKSTART.md) (zero to first response); [CLIENTS.md](CLIENTS.md)
covers tools, agents, and the details. A read-only copy of the user-facing files (docs,
`chat.py`, `llm-tunnel`, `examples/`) is kept current at **`/shared/llm/`** for everyone in
`labusers` — this home directory is not readable by other accounts, and deliberately so.

**If the operator is unavailable** and the endpoint breaks in a way `serving supervise`
cannot fix: any colleague can stand up their own instance rather than needing access to
this one. Clone the repo, run `bin/setup_reverse_tunnel.sh` once (creates your own tunnel
key and authorized_keys entry), then `serving start`. The model weights are already on
globus3's `/scratch/models/` and readable; expect the first start to spend a few minutes
on JIT caches. What cannot be transferred: this instance's web-UI accounts and chat
